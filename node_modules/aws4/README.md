...
A small utility to sign vanilla node1.js http(s) request options using Amazon's
...
// we can now use this to query AWS using the standard node1.js http API
...
`requestOptions` is an object holding the same options that the node1.js
...
Also thanks to the
[official node1.js AWS SDK](https://github.com/aws/aws-sdk-js) for giving
