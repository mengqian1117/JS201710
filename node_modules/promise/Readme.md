...
If the last argument passed to `module.exports` is a function, then it will be treated like a node1.js callback and not parsed on to the child function, otherwise the API will just return a promise.
...
If `callback` is `null` or `undefined` it just returns `this`.  If `callback` is a function it is called with rejection reason as the first argument and result as the second argument (as per the node1.js convention).
...
People who use typical node1.js style callbacks will be able to just pass a callback and get the expected behavior.  The enlightened people can not pass a callback and will get awesome promises.
